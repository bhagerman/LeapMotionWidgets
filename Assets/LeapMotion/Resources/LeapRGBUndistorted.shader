  ...
  SubShader {
    ...
    Pass {
      ...
      float4 colorCorrect(float texImageX, float texImageY) {
        ...
        float4 mygl_FragColor;
        
        mygl_FragColor.r = rfudge*output_lf_fudge.r + (1-rfudge)*output_lf.r;
        mygl_FragColor.g = gfudge*output_lf_fudge.g + (1-gfudge)*output_lf.g;
        mygl_FragColor.b = bfudge*output_lf_fudge.b + (1-bfudge)*output_lf.b;

        mygl_FragColor.r *= rscale;
        mygl_FragColor.g *= gscale;
        mygl_FragColor.b *= bscale;
       
        //float avgrgb = 0.33333*(input_lf.r + input_lf.g + input_lf.b) - 0.9*input_lf.a;
        //float threshold = min(1, avgrgb*100);
        //threshold *= threshold;
        //mygl_FragColor.rgb = output_lf_gray.rgb*(1 - threshold) + mygl_FragColor.rgb*(threshold);
       
        float gamma = 1.0/_GammaCorrection;
        //mygl_FragColor.rgb = show_ir > 0.5 ? float3(pow(ir_out, gamma)): pow(mygl_FragColor.rgb, float3(gamma));
        
        mygl_FragColor = float4(pow(mygl_FragColor.rgb, float(gamma)), 1.0);
        return mygl_FragColor;
      }
